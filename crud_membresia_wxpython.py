...
class MembresiaCRUD(wx.Panel):
    def __init__(self, parent, title):
        super(MembresiaCRUD, self).__init__(parent)
        ...
        columnas = ["ID Tarjeta", "Puntos", "Fecha Emisión", "Fecha Expiración"]
    ...
    def actualizar_lista(self):
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                self.lista.SetItem(index, 3, row[3])
        except Exception as e:
            wx.MessageBox(f"Error al cargar membresías: {str(e)}", "Error", wx.OK|wx.ICON_ERROR)
        finally:
            if conn:
                conn.close()
    ...
    def agregar(self, event):
        ...
        if all(valores):
            try:
                ...
                wx.MessageBox(f"Error al agregar: {str(e)}", "Error", wx.OK|wx.ICON_ERROR)
            finally:
                if conn:
                    conn.close()
    ...
    def actualizar(self, event):
        ...
        if idx >= 0:
            ...
            try:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "UPDATE membresia SET puntos=%s, fecha_emision=%s, fecha_expiracion=%s WHERE id_tarjeta=%s",
                ...
                wx.MessageBox(f"Error al actualizar: {str(e)}", "Error", wx.OK|wx.ICON_ERROR)
            finally:
                if conn:
                    conn.close()
    ...
    def eliminar(self, event):
        ...
        if idx >= 0:
            ...
            try:
                ...
                wx.MessageBox(f"Error al eliminar: {str(e)}", "Error", wx.OK|wx.ICON_ERROR)
            finally:
                if conn:
                    conn.close()
